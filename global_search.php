...
define ("DB_NAME","tpevent"); // set database name
...
$query = 'SELECT f_email FROM t_faculty WHERE f_email LIKE "%'.$user_input.'%" order by f_email LIMIT 3';
...
while($recResult = mysql_fetch_assoc($recSql)) {
  ...
  $json_arr["value"] = $recResult['f_email'];
